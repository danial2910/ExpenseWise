...
// Thresholds and colors mirror the imported design exactly: on-track uses
// the neutral slate-700 (not green) rather than a "success" color, since a
// budget with room left isn't a positive event worth celebrating — just
// the default state.
  ...
  <AppLayout title="Budgets" fab-label="Set overall budget" @fab="openOverallEdit">
    <div class="flex flex-col gap-6">
      ...
      >
        ...
        <p class="text-base font-semibold text-surface-900">Couldn't load budgets</p>
        <p class="text-sm text-surface-500 text-center max-w-sm">
          Something went wrong while fetching your budgets. Check your connection and try again.
        </p>
        <Button data-testid="budgets-retry-button" label="Retry" icon="pi pi-refresh" @click="retry" />
      </div>

      <!-- loading state -->
      <div v-else-if="loadState === 'loading'" data-testid="budgets-loading-skeleton" class="flex flex-col gap-4">
        <div class="bg-white border border-surface-200 rounded-lg p-6 h-24 animate-pulse" />
      ...
      <div v-else class="flex flex-col gap-6">
        ...
        >
          ...
          <p class="text-base font-semibold text-surface-900">No budgets set for {{ monthLabel }}</p>
          <p class="text-sm text-surface-500 text-center max-w-sm">
            Set a monthly limit overall or per category to see how your spending tracks against it.
          </p>
          <Button
    ...
    >
      <div class="flex flex-col gap-4">
        ...
        <div>
          ...
          <div
            class="flex items-baseline gap-2 px-4 py-3 border rounded-lg bg-surface-50"
            :class="amountError ? 'border-red-600' : 'border-surface-200'"
          >
      ...
      <template #footer>
        ...
        <Button data-testid="budget-save-button" label="Save" :loading="saving" @click="saveEditor" />
      </template>
    </Dialog>
  </AppLayout>
</template>
